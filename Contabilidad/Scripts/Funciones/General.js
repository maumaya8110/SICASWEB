...
function RenderCustomCheckBoxProgPago(value, extra) {
    return '<input type="checkbox" '+ extra + '></input>';
}
...
function SetDateTimeControl(e, h) {
    var datos = e.data.record;
    var celda = $(this);
    celda.empty();
    var dte = $("<div class='input-group date' id='dte_" + datos.CIDDOCUMENTO + "'><input type='text' class='form-control' /><span class='input-group-addon'><span class='glyphicon glyphicon-calendar'></span></span></div>");
    dte.data('record', datos);
    dte.appendTo(celda);
